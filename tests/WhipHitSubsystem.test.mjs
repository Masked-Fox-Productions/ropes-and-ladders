...
} from "../ropes_bp/scripts/util/RopeConstants.js";

function makeAttacker(heldId = "ropes:whip", location = { x: 0, y: 64, z: 0 }) {
...
function makeHurtEntity(location, particleSink, knockbackSink) {
  return {
    location,
    dimension: {
      spawnParticle(id, loc) { particleSink.push({ id, loc }); },
    },
    applyKnockback(horizontal, vertical) { knockbackSink?.push({ horizontal, vertical }); },
  };
}
...
describe("WhipHitSubsystem", () => {
  ...
  it("knocks the hit entity away from the attacker", () => {
    const knockbacks = [];
    const hurtEntity = makeHurtEntity({ x: 5, y: 64, z: 0 }, [], knockbacks);

    sub._handleHurt({
      damageSource: { damagingEntity: makeAttacker("ropes:whip", { x: 0, y: 64, z: 0 }) },
      hurtEntity,
    });

    assert.equal(knockbacks.length, 1);
    // entity is due +x of the attacker, so it should be pushed in +x with full horizontal force
    assert.ok(Math.abs(knockbacks[0].horizontal.x - WHIP_KNOCKBACK_HORIZONTAL) < 1e-9);
    assert.ok(Math.abs(knockbacks[0].horizontal.z) < 1e-9);
    assert.equal(knockbacks[0].vertical, WHIP_KNOCKBACK_VERTICAL);
  });

  it("falls back to the attacker's view direction when stacked on the target", () => {
    const knockbacks = [];
    const hurtEntity = makeHurtEntity({ x: 0, y: 64, z: 0 }, [], knockbacks);
    ...
    assert.equal(knockbacks.length, 1);
    // view direction is +z, so knockback is along +z
    assert.ok(Math.abs(knockbacks[0].horizontal.x) < 1e-9);
    assert.ok(Math.abs(knockbacks[0].horizontal.z - WHIP_KNOCKBACK_HORIZONTAL) < 1e-9);
  });
